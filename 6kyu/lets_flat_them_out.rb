...
#def flatten(hash)
#  hash.each_with_object({}) { |(key, value), flattened|
#    flattened[key] = value and next unless value.is_a?(Hash)
#    flattened[key] = ""    and next if value.empty?
#    flatten(value).each { |child_key, child_value|
