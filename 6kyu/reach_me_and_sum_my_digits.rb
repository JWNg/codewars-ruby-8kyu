...
def sumDig_nthTerm(initval, patternl, nthterm)
  times = (nthterm-1) / patternl.size
  remainder = (nthterm-1) % (patternl.size)
  sum = patternl.inject (:+)
  initval += (sum * times)
  i = 0
  until i == remainder do
    initval += patternl[i]
    i += 1
  end
  split_appart = initval.to_s.split("")
  total = 0
  split_appart.each {|digit| total += digit.to_i}
  total
end
